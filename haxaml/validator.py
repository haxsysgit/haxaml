...
def semantic_validate(frame: "FrameModel") -> SemanticValidationResult:
    ...
    # 0.8.0 is a fresh frontmatter-only schema slice. Body fields are not
    # interpreted by semantic validation until a later slice defines them.
...
def detect_missing_facts_fields(facts_path: str) -> list[str]:
    """Return facts body completeness findings for the current schema slice.

    0.8.0 defines frontmatter only, so there are no facts-body completeness
    checks yet. Future slices should add checks only after the fields are added
    to the schema.
    """
    load_yaml(facts_path)
    return []
